    ...
    <div className="space-y-8">
      ...
      <div>
        <h3 className="text-lg font-bold text-red-600 flex items-center gap-2 mb-4 border-b border-gray-100 pb-2">
          <XCircle size={20}/> Các từ làm sai ({logs.filter(l => !l.isCorrect).length})
        </h3>
        <div className="grid gap-4">
          {logs.filter(l => !l.isCorrect).map((log, idx) => (
            <div key={idx} className="flex justify-between items-start p-5 bg-red-50 rounded-xl border border-red-100 shadow-sm">
              <div>
                <p className="font-bold text-gray-900 text-lg">{log.q.word}</p>
                <div className="flex items-center gap-3 text-gray-500 mt-1 mb-2 text-sm">
                  <span>{log.q.pronunciation}</span>
                  <span className="px-2 py-0.5 bg-white border border-red-200 rounded text-xs font-semibold">{log.q.type}</span>
                  <StatusBadge status={log.q.status} />
                </div>
                <p className="text-base text-red-800 font-medium">{log.q.meaning}</p>
          ...
          {logs.filter(l => !l.isCorrect).length === 0 && <p className="text-gray-400 italic">Không có từ nào làm sai!</p>}
        </div>
      </div>
      ...
      <div>
        <h3 className="text-lg font-bold text-green-600 flex items-center gap-2 mb-4 border-b border-gray-100 pb-2">
          <CheckCircle2 size={20}/> Các từ làm đúng ({logs.filter(l => l.isCorrect).length})
        </h3>
        <div className="grid gap-4">
          {logs.filter(l => l.isCorrect).map((log, idx) => (
            <div key={idx} className="flex justify-between items-start p-5 bg-green-50 rounded-xl border border-green-100 shadow-sm">
              <div>
                <p className="font-bold text-gray-900 text-lg">{log.q.word}</p>
                <div className="flex items-center gap-3 text-gray-500 mt-1 mb-2 text-sm">
                  <span>{log.q.pronunciation}</span>
                  <span className="px-2 py-0.5 bg-white border border-green-200 rounded text-xs font-semibold">{log.q.type}</span>
                  <StatusBadge status={log.q.status} />
                </div>
