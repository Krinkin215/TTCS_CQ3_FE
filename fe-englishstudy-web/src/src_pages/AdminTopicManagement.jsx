...
import { Search, X, Filter, Plus, Edit2, Trash2, BookOpen, Eye, Check, ChevronRight, Copy, FolderInput, LogOut, MoreVertical, Gamepad2 } from 'lucide-react';
...
export default function AdminTopicManagement() {
  ...
  const openLessonWords = (lesson, topic) => {
    ...
    setIsWordSelectMode(false);
    setSelectedWordIds([]);
    setShowTopicLessonsModal(false); // Close previous
    ...
    <div className="p-8 bg-slate-50 min-h-screen">
      ...
      {showCreateTopicModal && (
        <div className="fixed inset-0 bg-cyan-950/70 z-[200] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-[1.5rem] w-full max-w-md p-6 shadow-2xl animate-in zoom-in duration-200">
      ...
      {showTopicWordsModal && activeTopic && (
        <div className="fixed inset-0 bg-cyan-950/70 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-6xl shadow-2xl animate-in zoom-in duration-200 overflow-hidden flex flex-col h-[90vh]">
            <div className="flex justify-between items-center p-5 border-b border-gray-100 bg-cyan-50/30">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="relative">
                  <button
                    onClick={() => setShowLessonFilterDropdown(v => !v)}
                    className={`flex items-center gap-2 px-3 py-2 border rounded-xl text-sm font-medium outline-none transition-colors ${
                      selectedLessonFilters.length > 0
                        ? 'border-cyan-400 bg-cyan-50 text-cyan-700'
                        : 'border-gray-200 text-gray-700 hover:border-cyan-300'
                    }`}
                  >
                    <Filter size={15}/>
                    {selectedLessonFilters.length > 0 ? `${selectedLessonFilters.length} bài` : 'Lọc bài học'}
                  </button>
                  {showLessonFilterDropdown && (
                    <div className="absolute top-full left-0 mt-2 w-60 bg-white border border-gray-200 rounded-2xl shadow-xl z-50 py-2 overflow-hidden">
                      <div className="px-3 pb-2 pt-1 border-b border-gray-100 flex justify-between items-center">
                        <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Chọn bài học</span>
                        {selectedLessonFilters.length > 0 && (
                          <button onClick={() => setSelectedLessonFilters([])} className="text-xs text-red-500 font-bold hover:underline">Xóa lọc</button>
                        )}
                      </div>
      ...
      {showTopicLessonsModal && activeTopic && (
        <div className="fixed inset-0 bg-cyan-950/70 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-[1.5rem] w-full max-w-3xl shadow-2xl animate-in zoom-in duration-200 flex flex-col h-[85vh]">
      ...
      {showLessonWordsModal && activeLesson && (
        <div className="fixed inset-0 bg-cyan-950/70 z-[150] flex items-center justify-center p-4 backdrop-blur-sm">
           <div className="bg-white rounded-2xl w-full max-w-6xl shadow-2xl animate-in zoom-in duration-200 overflow-hidden flex flex-col h-[90vh]">
            <div className="flex justify-between items-center p-5 border-b border-gray-100 bg-cyan-50/30">
              <div>
